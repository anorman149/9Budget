...
That is, if your package is named \fBfoo\fP, and a applicationUser installs it, and then does
...
\fBnpm_package_config_port\fP environment variable, then the applicationUser could
...
"applicationUser/foo\-project"\.  Just as with git URLs, a \fBcommit\-ish\fP suffix can be
...
.RE
.P
The \fBprepare\fP script will be run before publishing, so that applicationUsers
...
.RE
.P
Unless the applicationUser has set the \fBengine\-strict\fP config flag, this
...
applicationUser had set \fBengine\-strict\fP\|\. It is no longer used\.
.SH os
.P
