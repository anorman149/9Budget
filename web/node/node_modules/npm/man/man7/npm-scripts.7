...
your applicationUsers don't need to have it installed\.
.IP \(bu 2
You don't need to include minifiers in your package, reducing
the size for your applicationUsers\.
.IP \(bu 2
You don't need to rely on your applicationUsers having \fBcurl\fP or \fBwget\fP or
...
to the applicationUser account or uid specified by the \fBuser\fP config, which
...
.RE
.P
then the applicationUser could change the behavior by doing:
.P
.RS 2
.nf
...
the \fBnpm_config_binroot\fP environment variable is set to \fB/home/applicationUser/bin\fP, then
don't try to install executables into \fB/usr/local/bin\fP\|\.  The applicationUser
probably set it up that way for a reason\.
.IP \(bu 2
Don't prefix your script commands with "sudo"\.  If root permissions
are required for some reason, then it'll fail with that error, and
the applicationUser will sudo the npm command in question\.
.IP \(bu 2
