...
exist and can be written by the current applicationUser\.
.IP \(bu 2
...
policy of the CLI team that we recommend all applicationUsers run \fBnpm@latest\fP if they
...
.SS \fBnode \-v\fP
.P
For most applicationUsers, in most circumstances, the best version of Node will be the
...
in general, the npm team recommends that most applicationUsers run Node\.js LTS\.
...
.RS 0
.IP \(bu 2
Your cache must be readable and writable by the applicationUser running npm\.
.IP \(bu 2
Global package binaries must be writable by the applicationUser running npm\.
.IP \(bu 2
Your local \fBnode_modules\fP path, if you're running \fBnpm doctor\fP with a project
directory, must be readable and writable by the applicationUser running npm\.
