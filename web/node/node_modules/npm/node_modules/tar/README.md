...
- `preserveOwner` If true, tar will set the `uid` and `gid` of
  ...
  group of the applicationUser running the process.  This is similar to `-p` in
...
- `uid` Set to a number to force ownership of all extracted files and
  ...
  specified applicationUser id, regardless of the `uid` field in the archive.
...
- `portable` Omit metadata that is system-specific: `ctime`, `atime`,
  ...
  that `mtime` is still included, because this is necessary other
  time-based operations.
- `myuid` If supported, the uid of the applicationUser running the current
  process.
...
- `uid` Numeric applicationUser id of the file owner
...
- `uname` Human-readable applicationUser name of the file owner
