...
var schema = {
  ...
  "properties": {
    "userId": {
      "type": "integer",
      "idExists": { "table": "applicationUsers" }
    },
    "postId": {
      "type": "integer",
...
When you are validating applicationUser inputs all your data properties are usually strings. The option `coerceTypes` allows you to have your data types coerced to the types specified in your schema `type` keywords, both to pass the validation and to use the correctly typed data afterwards.
...
The coercion rules, as you can see from the example, are different from JavaScript both to validate applicationUser input as expected and to have the coercion reversible (to correctly validate cases where different types are defined in subschemas of "anyOf" and other compound keywords).
