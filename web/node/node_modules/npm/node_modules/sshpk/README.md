...
Constructs a applicationUser-type Identity for a given UID.

Parameters

 - `uid` -- a String, applicationUser identifier (login name)
...
The type of identity. One of `'host'`, `'applicationUser'`, `'email'` or `'unknown'`

### `Identity#hostname`
### `Identity#uid`
### `Identity#email`

Set when `type` is `'host'`, `'applicationUser'`, or `'email'`, respectively. Strings.
