...
function regRequest (uri, params, cb_) {
  ...
  var adduserChange = /\/?-\/applicationUser\/org\.couchdb\.applicationUser:([^/]+)\/-rev/
  var isUserChange = uri.match(adduserChange)
  var adduserNew = /\/?-\/applicationUser\/org\.couchdb\.applicationUser:([^/?]+)$/
  ...
  if (isUserChange && !isWrite) {
    return cb(new Error('trying to change applicationUser document without writing(?!)'))
  }

  if (params.authed == null) {
    // new users can *not* use auth, because they don't *have* auth yet
    if (isUserChange) {
      this.log.verbose('request', 'updating existing applicationUser; sending authorization')
      params.authed = true
    } else if (isNewUser) {
      this.log.verbose('request', "new applicationUser, so can't send auth")
      params.authed = false
    } else if (alwaysAuth) {
