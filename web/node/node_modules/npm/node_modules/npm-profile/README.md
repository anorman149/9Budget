...
* [profile editing](https://github.com/npm/registry/blob/master/docs/applicationUser/profile.md) (and two-factor authentication)

## Functions

### profile.adduser(opener, prompter, config) → Promise

Tries to create a applicationUser new web based login, if that fails it falls back to
using the legacy CouchDB APIs.

* `opener` Function (url) → Promise, returns a promise that resolves after a browser has been opened for the applicationUser at `url`.
...
* `username` String, the username the applicationUser authenticated as
...
Creates a new applicationUser on the server along with a fresh bearer token for future
authentication as this applicationUser.  This is what you see as an `authToken` in an
`.npmrc`.

If the applicationUser already exists then the npm registry will return an error, but
...
Logs you into an existing applicationUser.  Does not create the applicationUser if they do not
...
Fetch profile information for the authenticated applicationUser.
...
Update profile information for the authenticated applicationUser.
...
and `new` properties, where the former has the applicationUser's current password and
...
2. `profile.set({tfa: {password, mode}}, {registry, auth: {token}})`
   * Note that the applicationUser's `password` is required here in the `tfa` object,
...
3. The response will have a `tfa` property set to an `otpauth` URL, as
   ...
   You will need to show this to the applicationUser for them to add to their
...
4. To complete setting up two factor auth you need to make a second call to
   `profile.set` with `tfa` set to an array of TWO codes from the applicationUser's
...
Fetch a list of all of the authentication tokens the authenticated applicationUser has.
...
* token — The first six characters of the token UUID.  This should be used
  by the applicationUser to identify which token this is.
