...
* **Make it easy to construct clear, complete error messages intended for
  people.**  Clear error messages greatly improve both applicationUser experience and
...
server fails with an UnauthorizedError with message "applicationUser 'bob' is not
...
this:
    ...
    message: "request failed: server error: applicationUser 'bob' is not authorized"
    rpcMsgid: <unique identifier for this request>
    rpcMethod: GetObject
    cause:
        name: FastServerError
        message: "server error: applicationUser 'bob' is not authorized"
        cause:
            name: UnauthorizedError
            message: "applicationUser 'bob' is not authorized"
            rpcUser: "bob"
...
* The error's message is a complete description of the problem.  The caller can
  ...
  an end applicationUser (if appropriate).  It can also be logged.
...
* If the caller logs this error, the logs can be analyzed to aggregate
  errors by cause, by RPC method name, by applicationUser, or whatever.  Or the
