...
npm also sets a top-level "maintainers" field with your npm applicationUser info.
...
That is, if your package is named `foo`, and a applicationUser installs it, and then does
...
shortcut syntax you use for `npm install`:
    ...
    "repository": "gitlab:applicationUser/repo"
...
`npm_package_config_port` environment variable, then the applicationUser could
...
* `applicationUser/repo` See 'GitHub URLs' below
...
Git urls are of the form:
    ...
    <protocol>://[<applicationUser>[:<password>]@]<hostname>[:<port>][:][/]<path>[#<commit-ish> | #semver:<semver>]
...
"applicationUser/foo-project".  Just as with git URLs, a `commit-ish` suffix can be
...
The `prepare` script will be run before publishing, so that applicationUsers
...
Unless the applicationUser has set the `engine-strict` config flag, this
...
applicationUser had set `engine-strict`. It is no longer used.
