...
* You don't need to rely on your applicationUsers having `curl` or `wget` or
...
to the applicationUser account or uid specified by the `applicationUser` config, which
...
* Inspect the env to determine where to put things.  For instance, if
  the `npm_config_binroot` environment variable is set to `/home/applicationUser/bin`, then
  don't try to install executables into `/usr/local/bin`.  The applicationUser
