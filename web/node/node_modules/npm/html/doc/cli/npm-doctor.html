...
exist and can be written by the current applicationUser.</li>
...
policy of the CLI team that we recommend all applicationUsers run <code>npm@latest</code> if they
...
<p>For most applicationUsers, in most circumstances, the best version of Node will be the
...
in general, the npm team recommends that most applicationUsers run Node.js LTS.</p>
...
<ul>
<li>Your cache must be readable and writable by the applicationUser running npm.</li>
<li>Global package binaries must be writable by the applicationUser running npm.</li>
<li>Your local <code>node_modules</code> path, if you&#39;re running <code>npm doctor</code> with a project
directory, must be readable and writable by the applicationUser running npm.</li>
</ul>
