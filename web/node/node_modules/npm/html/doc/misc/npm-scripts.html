...
allow applicationUsers to avoid the confusing behavior of existing npm versions and only
...
your applicationUsers don&#39;t need to have it installed.</li>
<li>You don&#39;t need to include minifiers in your package, reducing
the size for your applicationUsers.</li>
<li>You don&#39;t need to rely on your applicationUsers having <code>curl</code> or <code>wget</code> or
...
<h2 id="applicationUser">USER</h2>
<p>If npm was invoked with root privileges, then it will change the uid
to the applicationUser account or uid specified by the <code>applicationUser</code> config, which
...
<pre><code>http.createServer(...).listen(process.env.npm_package_config_port)</code></pre><p>then the applicationUser could change the behavior by doing:</p>
...
the <code>npm_config_binroot</code> environment variable is set to <code>/home/applicationUser/bin</code>, then
don&#39;t try to install executables into <code>/usr/local/bin</code>.  The applicationUser
...
the applicationUser will sudo the npm command in question.</li>
...
<p id="footer">npm-scripts &mdash; npm@6.3.0</p>
